...
class ChargingScreen(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Button(btn_frame, text="Simulate Unplug", font=("Arial", 12, "bold"),
                  bg="#e67e22", fg="white", width=14, command=self.simulate_unplug).grid(row=1, column=1, padx=6, pady=8)
    ...
    def start_charging(self):
        ...
        self.is_charging = True
        self.db_acc = 0
        # set responsive local remaining timer and start ticks
        self.remaining = cb
        self.time_var.set(str(self.remaining))
    ...
    def simulate_unplug(self):
        ...
        if not uid:
            return
        # simulate current sensor detecting unplug: update DB and pause local charging loop
        write_user(uid, {"charging_status": "idle"})
        self.unplug_time = time.time()
        # cancel the active tick loop so the timer visibly pauses
        if self._tick_job is not None:
            try:
                self.after_cancel(self._tick_job)
            except Exception:
                pass
            self._tick_job = None
        # mark as not actively charging locally (grace checker will resume if charging returns)
        self.is_charging = False
        # start grace checker which will end session after UNPLUG_GRACE_SECONDS unless replugged
        self._check_unplug_grace()
