...
class HardwareGPIO:
    ...
    def tm1637_init_slot(self, slot_key: str):
        """Initialize or return a TM1637 display instance bound to a specific slot.
        Uses the pinmap tm1637.dio mapping to select the correct DIO pin per slot.
        Returns a wrapper with show_time(seconds) and set_brightness(level).
        """
        if not hasattr(self, '_tm_slots'):
            self._tm_slots = {}
        # return cached instance if present
        try:
            if slot_key in self._tm_slots:
                return self._tm_slots[slot_key]
        except Exception:
            pass
        clk = self.pinmap.get('tm1637', {}).get('clk')
        dio_map = self.pinmap.get('tm1637', {}).get('dio', {})
        dio_pin = dio_map.get(slot_key)
        # if no specific dio mapping, fallback to generic tm
        if dio_pin is None:
            return self.tm1637_init()
        # try to create a tm1637 library instance bound to this dio pin
        try:
            import tm1637 as _tm1637_lib
            disp = _tm1637_lib.TM1637(clk=clk, dio=dio_pin)
            class _WrapSlot:
                def __init__(self, d):
                    self.d = d
                def show_time(self, seconds: int):
            ...
            try:
                ...
                pass
            self._tm_slots[slot_key] = w
            return w
        except Exception:
            # fallback to local TM1637Display bound to dio_pin
            try:
                disp = TM1637Display(clk_pin=clk, dio_pin=dio_pin, gpio=self.gpio, mode=self.mode)
                self._tm_slots[slot_key] = disp
                try:
                ...
                # final fallback: return the generic tm instance
                return self.tm1637_init()
