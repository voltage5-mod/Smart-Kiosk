...
class HardwareGPIO:
    ...
    # TM1637 multi-slot driver support
    def tm1637_init(self, slot: str = 'slot1'):
        """Initialize TM1637 display for a specific slot.
        
        Returns a display object (or wrapper) with show_time(seconds) and set_brightness(level) methods.
        If slot is None or 'all', returns a dict of all available displays.
        """
        if slot is None or slot == 'all':
            # Return dict of all initialized displays
            if self.tm is None:
                self.tm = {}
            for s in ['slot1', 'slot2', 'slot3', 'slot4']:
                if s not in self.tm:
                    self.tm[s] = self.tm1637_init(s)
            return self.tm
        
        # Single slot initialization
        if isinstance(self.tm, dict):
            if slot in self.tm:
                return self.tm[slot]
        elif self.tm is not None:
            # Already have a single display object, return it if it's for this slot
            return self.tm
        ...
        try:
            ...
            class _Wrap:
                ...
                def show_time(self, seconds: int):
                    ...
                    ss = seconds % 60
                    try:
                        # Some libraries have `numbers(mm, ss)` helper
                        ...
                        print(f'[TM1637 {self.slot}] show_time error: {e}')
                ...
                def set_brightness(self, level: int):
                    ...
                    try:
                        ...
                        print(f'[TM1637 {self.slot}] set_brightness error: {e}')
            
            wrapper = _Wrap(disp, slot)
            wrapper.set_brightness(1)  # Low default brightness
            
            # Cache in dict
            if not isinstance(self.tm, dict):
                self.tm = {}
            self.tm[slot] = wrapper
            
            return wrapper
        
        except ImportError:
            # Fall back to internal TM1637Display driver
            display = TM1637Display(clk_pin=clk, dio_pin=dio, gpio=self.gpio, mode=self.mode)
            display.set_brightness(1)
            
            # Cache in dict
            if not isinstance(self.tm, dict):
                self.tm = {}
            self.tm[slot] = display
            
            return display
        
        except Exception as e:
            print(f'[TM1637] Failed to initialize {slot}: {e}')
            return None
