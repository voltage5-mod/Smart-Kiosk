...
class ChargingScreen(tk.Frame):
    ...
    def _poll_for_charging_start(self):
        """Simplified and more reliable charging detection"""
        self._wait_job = None
        ...
        try:
            ...
            # Debug output
            print(f"[CHG DETECT] Slot: {slot}, Amps: {amps:.3f}A, Threshold: {PLUG_THRESHOLD}A")
            
            # Simple state machine for detection
            if amps >= PLUG_THRESHOLD:
                self._charge_consecutive += 1
                print(f"[CHG DETECT] Above threshold - consecutive: {self._charge_consecutive}")
            else:
                self._charge_consecutive = 0
            ...
            if self._charge_consecutive >= CONFIRM_SAMPLES and not self.is_charging:
                print(f"[CHG DETECT] Starting charging session - detected {self._charge_consecutive} samples")
