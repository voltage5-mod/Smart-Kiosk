...
SERVICE_KEY = r"C:\Users\Asus\THESIS\Repository\kioskk\firebase-key.json"                  # placed in same folder
...
class KioskApp(tk.Tk):
    ...
    def _arduino_event_callback(self, event, value):
        ...
        """
        try:
            # Forward to any screen that implements handle_arduino_event
            for F, frame in list(self.frames.items()):
                try:
                    if hasattr(frame, 'handle_arduino_event'):
                        try:
                            frame.handle_arduino_event(event, value)
                        except Exception:
                            pass
                except Exception:
                    pass
...
class ChargingScreen(tk.Frame):
    ...
    def handle_arduino_event(self, event, value):
        """Handle Arduino events for charging screen (COIN_CHARGE, plug/unplug, etc.).
        Expected event: 'COIN_CHARGE' with peso value. We map peso -> seconds via COIN_MAP
        and persist identical to manual `insert_coin` behavior.
        """
        try:
            ev = (event or '').strip()
            if ev == 'COIN_CHARGE':
                ...
                try:
                    ...
                    write_user(uid, {'charge_balance': newbal})
                    try:
                        append_audit_log(actor=uid, action='insert_coin', meta={'amount': peso, 'added_seconds': add, 'new_balance': newbal})
                    except Exception:
                        pass
                    # if currently charging, update remaining display
                    if self.is_charging:
                        try:
                            self.remaining += add
                            self.time_var.set(str(self.remaining))
                            ...
                            pass
                    # record for UI summary and popups
                    try:
                        self.controller.record_coin_insert(uid, peso, add)
                    except Exception:
                        pass
                    try:
                        # show popup with added seconds
                        self.controller.show_coin_popup(uid, peso=peso, added_ml=None, total_ml=None)
                    except Exception:
                        pass
                    try:
